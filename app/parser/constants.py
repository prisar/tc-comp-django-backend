...
reserved = {'IF': 'IF',
            ...
            'TWENTY_PERCENT_POINT': 'TWENTY_PERCENT_POINT',
            'COMPONENT': 'COMPONENT',
            'LESS': 'LESS',
            'POWERFUL': 'POWERFUL',
            'IN': 'IN',
...
def read_parser_out_file():
    global states

    in_state = False
    num = -1
    with open(Config.PARSER_OUT_PATH, 'r') as f:
        for line in f:
            l = line.strip()
            if len(l) > 0:
                if not in_state:
